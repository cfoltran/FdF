...
#    Updated: 2019/02/07 15:04:16 by clfoltra         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
# Minilibx
LIBX_PATH	:= $(LIBDIR)/minilibx_macos
include $(LIBX_PATH)/include.mk
...
# Program linkage
$(NAME):	$(OBJ) $(LIBFT_PATH)/$(LIBFT) $(LIBX_PATH)/$(LIBX)
	@ echo $(ONGOINGCOL)[...]$(COLRESET)"	: "$@$(MOVELINE)
	@ $(LD) -o $(NAME) $(OBJ) $(LDFLAG) &> $(TMP) && \
		(echo $(CLEARLINE)$(LINKCOLOR)[fdf]$(COLRESET)"	: created"  ; \
...
$(LIBX_PATH)/$(LIBX):
	@ echo $(ONGOINGCOL)[...]$(COLRESET)"	: "$@$(MOVELINE)
	@ $(MAKE) -C $(LIBX_PATH) --no-print-directory > $(TMP) 2> $(TMP) && \
		(echo $(CLEARLINE)$(CYAN)[OK]$(COLRESET)"	: "$@ ; rm $(TMP)) || \
		(echo $(CLEARLINE)$(KOCOLOR)[KO]$(COLRESET)"	: "$@ ; \
			cat $(TMP) | sed -e "s/^/    /g" ; rm $(TMP) ; exit 1)
